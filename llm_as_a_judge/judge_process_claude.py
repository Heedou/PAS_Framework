...
class ClaudeClient:
    def __init__(self, config: Dict[str, Any]):
        ...
        self.response_format = config['prompt']['tools']
        ...
        try:
            async with session.post('https://api.anthropic.com/v1/messages',
                ...
                if response.status != 200:
                    ...
                    print(f" in idx : {idx}, API error {response.status}: {error_data}")
                    return idx, None
                ...
                for content in data.get('content', []):
                    if (content.get('type') == 'tool_use' and 
                        content.get('name') == 'answer_json'):
   
                        return idx, json.dumps(content['input'], ensure_ascii=False)
    ...
    async def process_chunk(self, question_column: str, session: aiohttp.ClientSession, df_chunk: pd.DataFrame, cache: List[Dict[str, Any]], answer_column: str) -> None:
        ...
        for idx, response in results:
            ...
            if response is not None and response.strip() != "" and response.strip() != "{}":
    ...
    async def generate(self, question_column:str, answer_column: str, test_mode: bool = False) -> pd.DataFrame:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            for chunk in tqdm(chunks, desc="Processing chunks"):
                ...
                print("claude limit 방지용...")
                time.sleep(self.config['model']['sleep_time'])
